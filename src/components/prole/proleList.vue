...
<div>
  <div class="search-box">
    <span>角色名称：</span><el-input  v-model="roleName"  class="search-ipt" placeholder="角色名称"></el-input>
    <span>角色类型：</span>
      <el-select
        v-model="roleType"
        clearable
...
<script>
import addDialog from "./proleAdd.vue";
import tip from "@/static/tip";
import addMenuDialog from "./proleAddMenu.vue";
export default {
  mixins: [tip],
  data() {
    return {
      ...
      url: this.common.serverUrl + "/system/queryRoleInfoPageByShop",
      roleName: "",
      roleType: "",
      registTime: "",
      updateTime: "",
  ...
  methods: {
    ...
    sureAddMenu(form) {
      ...
      this.common.httpPost("/system/addRoleMenu",params)
        .then(res=>{
          if (res.data.meta.code == 0) {
            this.messageInfo("编辑成功", "success");
          }else{
