...
<div>
  <div class="search-box">
    ...
    <span >姓名：</span><el-input v-model="userName" class="search-ipt" placeholder="请输入内容"></el-input>
    <span >用户类型：</span>
    <el-select
      v-model="userType"
      clearable
  ...
  <el-table
    ...
    <el-table-column
      ...
      <template slot-scope="scope">
          ...
          <el-button type="text" size="small" @click="handleEdit(3,scope.row)">重置密码</el-button>
        </template>
    </el-table-column>
  </el-table>
  <!--分页-->
...
export default {
  ...
  data() {
    return {
      ...
      url: this.common.serverUrl + "/system/queryUserInfoPage",
      id: "",
      userName: "",
      mobile: "",
      state: "",
  ...
  methods: {
    ...
    handleEdit(ids, row) {
      switch (ids) {
        ...
        case 2:
          ...
          break;
        case 3:
          this.resetPwd(row.id,row.mobile);
          break;
      }
    },
    //新增弹窗
    ...
    resetPwd(id,mobile) {
      this.confirmBox("请确认是否重置密码?", () => {
        let params = {
          id:id,mobile:mobile
        };
        this.common
          .httpPost("/system/resetUserPwd", params)
          .then(res => {
            if (res.data.meta.code == 0) {
              this.messageInfo("重置成功", "success");
            } else {
              this.messageInfo(res.data.meta.message, "warning");
            }
          })
